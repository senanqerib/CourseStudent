<%-- 
    Document   : header
    Created on : Oct 3, 2017, 2:34:45 PM
    Author     : qeribli_s
--%>

  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <script src="js/jquery-latest.min.js" type="text/javascript"></script>
  <script src="js/script.js"></script>
  <link rel="stylesheet" href="css/menu.css">
<div id='cssmenu'>
<ul>
   <li class='active'><a href='success.jsp?student=1'><span>Students</span></a></li>
   <li class='hover'><a href='success.jsp?teacher=1'><span>Teachers</span></a></li>
   <li class='hover'><a href='member.jsp?action=addstudent'><span>Add Student</span></a></li>
   <li class='hover'><a href='member.jsp?action=addteacher'><span>Add Teacher</span></a></li>
   <li><a href='reg.jsp'><span>Add New User</span></a></li>
   <li><a href='edit_user.jsp'><span>Edit User</span></a></li>
   <li><a href='sms_history.jsp'><span>SMS History</span></a></li>
   <li><a href='logout.jsp'><span>Log Out</span></a></li>
</ul>
</div>
